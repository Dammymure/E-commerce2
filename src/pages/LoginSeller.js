...
const LoginSeller = () => {
 ...
 // To navigate to a page after login
 ...
 const handleSubmit = (e) => {
  ...
  if (seller.email === "" || seller.password === "") {
   ...
   axios.post(`${process.env.REACT_APP_API_URL}/seller/login`, seller)
    .then((response) => {
     console.log(response.data.msg)
     localStorage.setItem("seller", JSON.stringify(response.data))

     if (response.data.msg === "You have successfully logged IN") {
      swal("Successful Login", response.data.msg, "success")
      navigate("/sellerdashboard")
     }
     else {
      swal("Invalid Email/Password", "Sorry your details are wrong", "error")
     }
    })
    .catch((err) => {
     console.log(err)
    ...
    <div class="container-fluid register">
     <div class="row login">
      <div class="col-md-3 register-left">
       ...
       <h3>Welcome</h3>
       <p>You don't have a seller account? Create one NOW!</p>
       <Link to="/registerseller" className="btn btn-light">Create a seller account</Link>
       <br />
      </div>
      ...
      <form onSubmit={handleSubmit} class="form_container  ">
       ...
       <div class="input_container">
        ...
        <input placeholder="name@mail.com" title="Inpit title"
         type="text"
         class="input_field"
         id="email_field"
         name='email'
